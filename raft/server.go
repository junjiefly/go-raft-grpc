...
type Server interface {
	...
	RequestNewVote(req *NewVoteRequest) *NewVoteResponse
...
func NewServer(name string, path string, transporter Transporter, stateMachine StateMachine, ctx interface{}, connectionString string) (Server, error) {
	...
	go func() {
		_ = s.StartGrpcServer(name)
	}()
	return s, nil
}

func (s *server) StartGrpcServer(grpcAddr string) error {
	var err error
	...
	opts = append(opts,
		...
		grpc.KeepaliveEnforcementPolicy(keepalive.EnforcementPolicy{MinTime: client.KeepaliveTime / 2, PermitWithoutStream: client.PermitWithoutStream}))
	gs := grpc.NewServer(opts...)
	s.gRpc = gs
	protobuf.RegisterRaftServerServer(gs, s)
...
// hekh. deadlock with peer.mutex at sendAppendEntries.
// change to read lock
...
func (s *server) followerLoop() {
	...
	for s.State() == Follower {
		...
		case e := <-s.c:
			...
			case *NewVoteRequest:
				ok := false
				//	elapsedTime := time.Duration(float64(electionTimeout)*2)
				//	s.DispatchEvent(newEvent(ElectionTimeoutThresholdEventType, elapsedTime, nil))
				if s.promotable() {
					s.setState(Candidate)
					ok = true
				}
				e.returnValue = &NewVoteResponse{Ok: ok}
			default:
				err = NotLeaderError
			}
			// Callback to event.
...
func (s *server) RequestNewVote(req *NewVoteRequest) *NewVoteResponse {
	ret, _ := s.send(req)
	resp, _ := ret.(*NewVoteResponse)
	return resp
}
...
var ErrRaftServerNotReady = errors.New("raft server not ready")

func (s *server) IsLeader() bool {
	return s.Leader() == s.Name()
}

func (s *server) PeerList() (members []string) {
	peers := s.Peers()
	for _, p := range peers {
...
func (s *server) ClusterStatusHandler(ctx cctx.Context, req *protobuf.ClusterStatusRequest) (*protobuf.ClusterStatusResponse, error) {
	ret := &protobuf.ClusterStatusResponse{
		IsLeader: s.IsLeader(),
		Peers:    s.PeerList(),
	}
	ret.Leader = s.Leader()
...
func (s *server) ClusterJoinHandler(ctx cctx.Context, req *protobuf.ClusterJoinRequest) (*protobuf.ClusterJoinResponse, error) {
	ret := &protobuf.ClusterJoinResponse{}
	...
	if _, err := s.Do(command); err != nil {
		...
		case NotLeaderError:
			leader := s.Leader()
			err = s.transporter.SendClusterJoinRequest(s, leader, command)
			if err != nil {
...
func (s *server) ClusterLeaveHandler(ctx cctx.Context, req *protobuf.ClusterLeaveRequest) (*protobuf.ClusterLeaveResponse, error) {
	ret := &protobuf.ClusterLeaveResponse{}
	...
	if _, err := s.Do(command); err != nil {
		...
		case NotLeaderError:
			leader := s.Leader()
			err = s.transporter.SendClusterLeaveRequest(s, leader, command)
			if err != nil {
				ret.ErrMsg = err.Error()
			}
		default:
...
func (s *server) AppendEntriesRequestHandler(ctx cctx.Context, req *protobuf.AppendEntriesRequest) (*protobuf.AppendEntriesResponse, error) {
	traceln(s.Name(), "RECV /appendEntries")
	...
	resp := s.AppendEntries(request)
	if resp == nil {
		return nil, errors.New("Failed creating response.")
	}
	reply := &protobuf.AppendEntriesResponse{
		Term:        resp.Term(),
		Index:       resp.Index(),
...
func (s *server) VoteRequestHandler(ctx cctx.Context, req *protobuf.RequestVoteRequest) (*protobuf.RequestVoteResponse, error) {
	traceln(s.Name(), "RECV /requestVote")
	...
	resp := s.RequestVote(request)
	if resp == nil {
		return nil, errors.New("Failed creating response.")
	}
	reply := &protobuf.RequestVoteResponse{
		Term:        resp.Term,
...
func (s *server) NewVoteHandler(ctx cctx.Context, req *protobuf.NewVoteRequest) (*protobuf.NewVoteResponse, error) {
	traceln(s.Name(), "RECV /newVote")
	request := &NewVoteRequest{}
	resp := s.RequestNewVote(request)
	if resp == nil {
		return nil, errors.New("Failed creating response.")
	}
	reply := &protobuf.NewVoteResponse{
		Ok: resp.Ok,
	}
	return reply, nil
}

func (s *server) SnapshotRequestHandler(ctx cctx.Context, req *protobuf.SnapshotRequest) (*protobuf.SnapshotResponse, error) {
	traceln(s.Name(), "RECV /snapshot")
	request := &SnapshotRequest{
		LeaderName: req.LeaderName,
		LastIndex:  req.LastIndex,
		LastTerm:   req.LastTerm,
	}
	resp := s.RequestSnapshot(request)
	if resp == nil {
		return nil, errors.New("Failed creating response.")
	}
	reply := &protobuf.SnapshotResponse{
		Success: resp.Success,
	}
	return reply, nil
}

func (s *server) SnapshotRecoveryRequestHandler(ctx cctx.Context, req *protobuf.SnapshotRecoveryRequest) (*protobuf.SnapshotRecoveryResponse, error) {
	traceln(s.Name(), "RECV /snapshotRecovery")
	request := &SnapshotRecoveryRequest{
		LeaderName: req.LeaderName,
		LastIndex:  req.LastIndex,
		LastTerm:   req.LastTerm,
		State:      req.State,
	...
	for _, v := range req.Peers {
		request.Peers = append(request.Peers, &Peer{Name: v.Name, ConnectionString: v.ConnectionString})
	}
	resp := s.SnapshotRecoveryRequest(request)
	if resp == nil {
		return nil, errors.New("Failed creating response.")
	}
	reply := &protobuf.SnapshotRecoveryResponse{
		Term:        resp.Term,
		Success:     resp.Success,
		CommitIndex: resp.CommitIndex,
	}
